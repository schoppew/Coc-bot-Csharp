...
namespace CoC.Bot.Tools
{
    public static class KeyboardHelper
    {
        ...
        {
            SendVirtualKey(BlueStacksHelper.GetBlueStackWindowHandle(), vk);
        }

        static public void SendToBS(string message)
        {
            Send(BlueStacksHelper.GetBlueStackWindowHandle(), message);
        }
        ...
        static public void NotePadTest()
        {
            ...
            {
                AdvancedMode = false;
                Send(hWnd, "This is a try... does it work?\nAnd what about exotic characters like: àçéèù?");
            }
        }

        static public void BSTest()
        {
            IntPtr hWnd = BlueStacksHelper.GetBlueStackWindowHandle();//Win32.FindWindow("Notepad", null);
            AdvancedMode = true;
            if (hWnd == IntPtr.Zero)
                MessageBox.Show("You should run BlueStack first before trying this test");
            else
            {
                Send(hWnd, "1> Does it work? And what for 'éàè'?");
            }
        }
